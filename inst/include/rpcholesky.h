// Main header file for rpcholesky package
#ifndef RPCHOLESKY_H
#define RPCHOLESKY_H
...
// Define namespace for rpcholesky package
namespace rpcholesky {
 ...
 */
inline arma::uvec sample_with_replacement_optimized(const int n, const int b, const arma::vec &weights) {
 ...
 */
inline Rcpp::List rejection_cholesky(arma::mat H) {
  int b = H.n_rows;
  if (H.n_rows != H.n_cols)
    Rcpp::stop("rejection_cholesky requires a square matrix");
  if (arma::trace(H) <= 0)
    Rcpp::stop("rejection_cholesky requires a strictly positive trace");
 ...
 */
inline Rcpp::List accelerated_rpcholesky(const arma::mat &A, int k, SEXP b_in, double stoptol, bool verbose) {
  if (A.n_rows != A.n_cols)
    Rcpp::stop("Matrix A must be square.");
  int n = A.n_rows;
  if (k <= 0 || k > n)
    Rcpp::stop("Invalid rank k.");

  // Initialize the diagonal residual and original trace.
  arma::vec diags = A.diag();
  double orig_trace = arma::sum(diags);
  if (orig_trace <= 0)
    Rcpp::stop("Matrix A must have a positive trace.");

  // Determine batch size b: if "auto", set b = ceil(k/10)
  int b;
  bool auto_b = false;
  if (TYPEOF(b_in) == STRSXP) {
    std::string b_str = Rcpp::as<std::string>(b_in);
    if (b_str == "auto") {
      b = std::ceil(k / 10.0);
      auto_b = true;
    } else {
      Rcpp::stop("Invalid b value. Use an integer or 'auto'.");
    }
  } else {
    b = Rcpp::as<int>(b_in);
  }
  ...
  while (counter < k) {
    ...
    Rcpp::List rej_res = rejection_cholesky(H);
    arma::mat L = rej_res["L"];
    arma::uvec accepted = rej_res["idx"];
    ...
    if (!solved)
      Rcpp::stop("Linear system could not be solved.");
    ...
    if (verbose) {
      Rcpp::Rcout << "Accepted " << num_sel << " / " << b << std::endl;
    }
  }

  return Rcpp::List::create(
    Rcpp::Named("G") = G,
    Rcpp::Named("idx") = arr_idx,
